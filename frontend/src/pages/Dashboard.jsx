...
const Dashboard = () => {
  ...
  //Update the gauge when when time frame changes
  useEffect(() => {
    const maxValues = {
  ...
  }, [currentTimeFrameData, timeFrame]); //The graph will be filled according to this data.
    ...
    <div className={dashboardStyles.container}>
      ...
      <div className={dashboardStyles.summaryGrid}>
          ...
          additionalContent={
            <div className="flex items-center gap-2 mt-2 text-sm">
              <span className={dashboardStyles.balanceBadge}>
                +R{Math.round(displayIncome).toLocaleString()}
              </span>
              <span className={dashboardStyles.expenseBadge}>
                -R{Math.round(displayExpenses).toLocaleString()}
              </span>
            </div>
          }
        />
        <FinancialCard
          ...
          value={`R${Math.round(displayExpenses).toLocaleString()}`}
          additionalContent={
            <div
        ...
        <FinancialCard
          ...
          value={`R${Math.round(displaySavings).toLocaleString()}`}
          additionalContent={
